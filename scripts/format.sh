...
if ! [[ -e "$ROOT/.git/refs/remotes/upstream" ]]; then
    git remote add 'upstream' 'https://github.com/alibaba/EasyRL.git'
fi
...
format_all() {
    yapf --diff "${YAPF_FLAGS[@]}" "${YAPF_EXCLUDES[@]}" demo easy_rl tests
    #yapf --in-place "${YAPF_FLAGS[@]}" "${YAPF_EXCLUDES[@]}" demo easy_rl tests
}
